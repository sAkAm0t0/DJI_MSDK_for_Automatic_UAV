...
import com.example.djimsdkproject.data.DEFAULT_STR
import dji.v5.utils.common.LogUtils
import dji.v5.manager.areacode.AreaCodeChangeListener
import dji.v5.manager.areacode.AreaCodeData
import dji.v5.network.IDJINetworkStatusListener
import com.example.djimsdkproject.data.MSDKInfo
import com.example.djimsdkproject.data.NO_NETWORK_STR
import com.example.djimsdkproject.data.ONLINE_STR
...
class MSDKInfoVm: DJIViewModel() {
    ...
    val msdkInfo: MutableLiveData<MSDKInfo> = MutableLiveData<MSDKInfo>()
    val mainTitle: MutableLiveData<String> = MutableLiveData<String>()
    ...
    init {
        ...
        msdkInfo.value?.buildVer = msdkInfoModel.getBuilderVersion()
        ...
        areaCodeChangeListener = AreaCodeChangeListener { _, changed: AreaCodeData ->
    ...
    fun initListener() {
        if(!SDKManager.getInstance().isRegistered || isInited.getAndSet(true)) {
            return
        }
